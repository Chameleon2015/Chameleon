...
RHBase = 'http://kodistream.info/repo/Main%20XML.xml'
...
def GetSublinks(name,url,iconimage,fanart):
    ...
    if c==1:
        try:
         ...
         rNo=dialog.select('RobinHood Select A Source', List)
         if rNo>=0:
             rName=str(List[rNo])
             rURL=str(ListU[rNo])
...
#hakamac code
    ...
    progress.create('Robinhood Searching Please wait',' ')
...
def urlsolver(url):
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification(RobinHood,Please enable Update Commonresolvers to Play in Settings. - ,10000)")

    resolved=genesisresolvers.get(url).result
    if url == resolved or resolved is None:
        #import
        xbmc.executebuiltin("XBMC.Notification(RobinHood,Using Urlresolver module.. - ,5000)")
        import urlresolver
...
def search(site_name,search_term=None):
    ...
    if 'youtube' in site_name:
        ...
        if res == 'True':
            ...
            xbmc.executebuiltin("XBMC.Notification(RobinHood,No IMDB match found ,7000,"+icon+")")
